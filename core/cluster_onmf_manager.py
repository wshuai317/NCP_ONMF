...
class ClusterONMFManager(ResultManager):
    ...
    def push_iters(self, penalty = None, iters = None):
	    ...
	    if penalty in self.iters_used.keys():
            	self.iters_used[penalty] += iters
	    else: self.iters_used[penalty] = iters
	...
	"""
        ...
        self.time_all[len(self.time_all)] = time_all
        self.time_comm[len(self.time_comm)] = time_comm
    ...
    def push_H_norm_ortho(self, val = None):
        ...
        if val is None: # if no input, just compute it using the last H
            ...
            H = np.asmatrix(H)
            (ha, hb) = H.shape
            #norm_H1 = np.asmatrix(np.diag(np.diag(H * H.transpose()) ** (-0.5))) * H
	    norm_H2 = normalize(H, norm = 'l2', copy = True, axis = 1)
	    norm_H2 = np.asmatrix(norm_H2)
            val = LA.norm(norm_H2 * norm_H2.transpose() - np.eye(ha), 'fro') / (ha * hb)
    ...
    def calculate_cluster_quality(self, true_labels = None):
        ...
        self.Purity[len(self.Purity)] = calculate_purity(cls_assignments, true_labels)
        self.ARI[len(self.ARI)] = calculate_rand_index(cls_assignments, true_labels)
        self.ACC[len(self.ACC)] = calculate_accuracy(cls_assignments, true_labels)
        self.NMI[len(self.NMI)] = calculate_NMI(cls_assignments, true_labels)
    ...
    def write_cluster_quality(self, file_path):
        ...
        with open(file_path, mode = 'wb') as csv_file:  # open the file, if not exist, create it
